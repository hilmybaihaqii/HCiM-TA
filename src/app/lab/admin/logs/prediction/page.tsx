'use client';

import React, { useEffect, useState, useMemo } from 'react';
import { api } from '@/lib/api';
import AdminTable from '../../components/AdminTable';
import { BrainCircuit, Search, Clock, ActivitySquare, Fingerprint, User, Zap, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const PAGE_SIZE = 50;

// Urutan HARUS sama persis dengan yang di-expect model AI
const FEATURE_NAMES = [
  'qNet', 'dvdtmax', 'vmax', 'vrest', 'APD50', 'APD90',
  'max_dv', 'camax', 'carest', 'CaTD50', 'CaTD90'
] as const;

type ProbabilityItem = { confidence: number; label: string };
type ProbabilitiesShape = ProbabilityItem[] | Record<string, number> | null;

interface UserData {
  id: string;
  display_name?: string;
  email?: string;
}

interface PredictionLog {
  id?: string;
  user_id?: string | null;
  input?: number[];
  predicted_tier?: string;
  probabilities?: ProbabilitiesShape;
  latency_ms?: number;
  created_at?: string;
  // Hasil join dengan /admin/users
  user_data?: UserData | null;
}

export default function PredictionLogsPage() {
  const [logs, setLogs] = useState<PredictionLog[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);

  // Pagination state
  const [page, setPage] = useState(0); // 0-indexed
  const [total, setTotal] = useState(0);

  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const offset = page * PAGE_SIZE;

  useEffect(() => {
    let isMounted = true;

    const fetchLogsAndUsers = async () => {
      setIsLoading(true);
      try {
        const [logsResponse, usersResponse] = await Promise.all([
          api(`/admin/prediction-logs?limit=${PAGE_SIZE}&offset=${offset}`),
          api("/admin/users?include_deleted=true")
        ]);

        if (!isMounted) return;

        const logsData = (logsResponse as { items?: PredictionLog[] }).items || [];
        const totalCount = (logsResponse as { total?: number }).total ?? logsData.length;
        const usersData = (usersResponse as { items?: UserData[] }).items || [];

        const userMap = new Map<string, UserData>();
        usersData.forEach(user => {
          if (user.id) userMap.set(user.id, user);
        });

        const enrichedLogs = logsData.map(log => ({
          ...log,
          user_data: log.user_id ? userMap.get(log.user_id) || null : null
        }));

        setLogs(enrichedLogs);
        setTotal(totalCount);
        setExpandedId(null); // tutup dropdown yang mungkin lagi kebuka pas pindah halaman
      } catch (error: unknown) {
        if (!isMounted) return;
        console.error("Failed to fetch prediction logs:", error);
        setErrorMsg("Failed to retrieve prediction records. Please check your connection.");
      } finally {
        if (isMounted) setIsLoading(false);
      }
    };

    fetchLogsAndUsers();

    return () => {
      isMounted = false;
    };
  }, [offset]);

  const filteredLogs = useMemo(() => {
    if (!searchQuery.trim()) return logs;

    const query = searchQuery.toLowerCase();
    return logs.filter(
      log =>
        (log.id || '').toLowerCase().includes(query) ||
        (log.user_id || '').toLowerCase().includes(query) ||
        (log.user_data?.display_name || '').toLowerCase().includes(query) ||
        (log.user_data?.email || '').toLowerCase().includes(query) ||
        (log.predicted_tier || '').toLowerCase().includes(query)
    );
  }, [searchQuery, logs]);

  const getTopConfidence = (probs: ProbabilitiesShape): number | null => {
    if (!probs) return null;

    if (Array.isArray(probs)) {
      if (probs.length === 0) return null;
      return Math.max(...probs.map(p => p.confidence));
    }

    const values = Object.values(probs);
    if (values.length === 0) return null;
    return Math.max(...values);
  };

  const columns = [
    {
      header: "Timestamp",
      className: "w-48",
      accessor: (log: PredictionLog) => {
        let dateStr = 'Unknown Time';

        if (log.created_at) {
          const cleanDateStr = log.created_at.replace(/^\+00/, '');
          const parsedDate = new Date(cleanDateStr);

          if (!isNaN(parsedDate.getTime())) {
            dateStr = parsedDate.toLocaleString();
          }
        }

        return (
          <div className="flex items-center gap-2 text-muted">
            <Clock className="w-3.5 h-3.5 shrink-0" />
            <span className="font-mono text-[10px] uppercase tracking-widest">{dateStr}</span>
          </div>
        );
      }
    },
    {
      header: "System Tracking",
      accessor: (log: PredictionLog) => {
        const safeLogId = log.id || 'UNKNOWN';
        const isExpanded = expandedId === safeLogId;
        const hasInput = Array.isArray(log.input) && log.input.length > 0;

        const displayName = log.user_data?.display_name
          || (log.user_id ? null : 'Anonymous'); // null user_id -> anonymous, ada user_id tapi gak match -> fallback ID di bawah
        const subLabel = log.user_data?.email
          || (log.user_id ? `ID: ${log.user_id.split('-')[0]}...` : 'Unauthenticated request');

        return (
          // "relative" penting: supaya dropdown di bawah bisa "float" (position: absolute)
          // relatif terhadap cell ini, bukan ikut alur dokumen (jadi gak dorong row lain)
          <div className="relative">
            <button
              type="button"
              onClick={() => hasInput && setExpandedId(isExpanded ? null : safeLogId)}
              className={`flex flex-col gap-1.5 text-left ${hasInput ? 'cursor-pointer' : 'cursor-default'}`}
              disabled={!hasInput}
            >
              <div className="flex items-center gap-2" title={log.user_id ? `Full User ID: ${log.user_id}` : 'Anonymous / unauthenticated request'}>
                <div className="w-5 h-5 rounded bg-foreground/5 flex items-center justify-center shrink-0">
                  <User className="w-3 h-3 text-muted" />
                </div>
                <span className="text-xs font-mono text-foreground font-medium">
                  {displayName || 'Unknown User'}
                </span>
                {hasInput && (
                  <ChevronDown
                    className={`w-3 h-3 text-muted transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                  />
                )}
              </div>
              <div className="flex items-center gap-2 pl-7">
                <span className="text-[10px] font-mono text-muted">
                  {subLabel}
                </span>
              </div>
            </button>

            {/* Dropdown parameter input - FLOATING, tidak mendorong layout row lain */}
            <AnimatePresence>
              {isExpanded && hasInput && (
                <motion.div
                  initial={{ opacity: 0, y: -4 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -4 }}
                  transition={{ duration: 0.15 }}
                  className="absolute top-full left-0 mt-1.5 z-20 w-[85vw] max-w-72 sm:w-72 p-2.5 rounded-lg bg-surface-white border border-foreground/10 shadow-lg grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1"
                >
                  {log.input!.map((value, idx) => (
                    <div key={idx} className="flex items-center justify-between gap-2 text-[10px] font-mono">
                      <span className="text-muted uppercase tracking-wide">
                        {FEATURE_NAMES[idx] ?? `param_${idx}`}
                      </span>
                      <span className="text-foreground font-medium">{value}</span>
                    </div>
                  ))}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        );
      }
    },
    {
      header: "Latency",
      className: "w-24",
      accessor: (log: PredictionLog) => {
        const latency = typeof log.latency_ms === 'number' ? log.latency_ms : null;
        const isSlow = latency !== null && latency > 5000;

        return (
          <div className={`flex items-center gap-1.5 ${isSlow ? 'text-amber-600' : 'text-muted'}`}>
            <Zap className="w-3 h-3 shrink-0" />
            <span className="font-mono text-[10px]">
              {latency !== null ? `${latency} ms` : '—'}
            </span>
          </div>
        );
      }
    },
    {
      header: "Prediction Output",
      className: "text-right",
      accessor: (log: PredictionLog) => {
        const rawTierSource = log.predicted_tier || 'UNKNOWN_TIER';
        const tierMatch = rawTierSource.match(/(high|intermediate|low)/i);
        const rawTier = (tierMatch ? tierMatch[1] : rawTierSource).toUpperCase();

        const getTierColor = (t: string) => {
          if (t.includes('HIGH')) return 'bg-rose-500/10 text-rose-600 border-rose-500/20';
          if (t.includes('LOW')) return 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20';
          if (t.includes('INTERMEDIATE')) return 'bg-amber-500/10 text-amber-600 border-amber-500/20';
          return 'bg-foreground/5 text-muted border-foreground/10';
        };

        const topConfidence = getTopConfidence(log.probabilities ?? null);

        return (
          <div className="flex items-center justify-end gap-2">
            {topConfidence !== null && (
              <span className="text-[9px] font-mono text-muted">
                {(topConfidence * 100).toFixed(0)}%
              </span>
            )}
            <span className={`px-3 py-1.5 text-[10px] rounded-md uppercase tracking-widest font-mono font-medium flex items-center gap-2 border shadow-sm ${getTierColor(rawTier)}`}>
              <ActivitySquare className="w-3.5 h-3.5" />
              {rawTier}
            </span>
          </div>
        );
      }
    }
  ];

  return (
    <div className="flex flex-col gap-6 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-medium tracking-tight text-foreground flex items-center gap-2">
            Prediction Records
          </h2>
          <p className="text-[11px] font-mono text-muted uppercase tracking-widest mt-1">
            Total Inferences: {total} &middot; Page {page + 1} of {totalPages}
          </p>
        </div>

        <div className="relative w-full sm:w-72">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search className="h-4 w-4 text-muted" />
          </div>
          <input
            type="text"
            placeholder="Search by Name, Email, ID or Tier..."
            value={searchQuery}
            onChange={(e) => {
              setSearchQuery(e.target.value);
              setExpandedId(null);
              setPage(0); // reset ke halaman pertama tiap kali user mulai search baru
            }}
            className="w-full bg-surface-white/40 border border-foreground/10 text-foreground text-xs rounded-full pl-10 pr-4 py-2.5 focus:outline-none focus:border-foreground/30 focus:ring-1 focus:ring-foreground/30 transition-all placeholder:text-muted/70 backdrop-blur-sm"
          />
        </div>
      </div>

      <AnimatePresence>
        {errorMsg && (
          <motion.div
            initial={{ opacity: 0, y: -10, height: 0 }}
            animate={{ opacity: 1, y: 0, height: 'auto' }}
            exit={{ opacity: 0, y: -10, height: 0 }}
            className="overflow-hidden"
          >
            <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-xl text-rose-500 text-xs text-center font-mono shadow-sm">
              [SYSTEM_ERROR] {errorMsg}
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* overflow-x-auto: di layar sempit, tabel bisa di-scroll horizontal
          alih-alih kolom saling numpuk/kepencet */}
      <div className="w-full overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
        <div className="min-w-[640px]">
          <AdminTable
            columns={columns}
            data={filteredLogs}
            isLoading={isLoading}
            emptyMessage={searchQuery ? "No matching prediction records found." : "No prediction data available."}
          />
        </div>
      </div>

      {/* Pagination controls */}
      <div className="flex items-center justify-between pt-2">
        <span className="text-[10px] font-mono text-muted uppercase tracking-widest">
          Showing {logs.length === 0 ? 0 : offset + 1}&ndash;{offset + logs.length} of {total}
        </span>
        <div className="flex items-center gap-2">
          <button
            onClick={() => page > 0 && setPage(p => p - 1)}
            disabled={page === 0 || isLoading}
            className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-mono uppercase tracking-widest rounded-full border border-foreground/10 text-foreground disabled:opacity-30 disabled:cursor-not-allowed hover:bg-foreground/5 transition-colors"
          >
            <ChevronLeft className="w-3.5 h-3.5" />
            Prev
          </button>
          <button
            onClick={() => page < totalPages - 1 && setPage(p => p + 1)}
            disabled={page >= totalPages - 1 || isLoading}
            className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-mono uppercase tracking-widest rounded-full border border-foreground/10 text-foreground disabled:opacity-30 disabled:cursor-not-allowed hover:bg-foreground/5 transition-colors"
          >
            Next
            <ChevronRight className="w-3.5 h-3.5" />
          </button>
        </div>
      </div>
    </div>
  );
}